...
- `FluentUI`（[zhuzichu520/FluentUI](https://github.com/zhuzichu520/FluentUI)，MIT 协议）— 提供 Windows 11 Fluent Design 风格 QML 控件
...
|---|---|
| **操作系统** | Windows 10+ |
| **Qt 6.5.3** | `msvc2019_64` 架构，含以下模块：`Quick` `Qml` `Network` `Widgets` `PrintSupport` `Svg` |
| **Visual Studio 2022** | 含 MSVC v143 工具链 |
| **Git Bash** | Git for Windows 自带，脚本依赖 bash |
| **网络** | 首次构建需从 GitHub 拉取 FluentUI（约 11 MB） |
...
脚本自动完成：下载 FluentUI → 打兼容补丁 → CMake 配置 → 编译 → windeployqt 部署。

### 启动界面

```bash
# 主界面（Unity 3D 嵌入）
QtUI\build5\Release\appLiftingTwinUI.exe

# 系统运行监控（链路状态 + 风险事件归档与追溯复盘）
QtUI\build5\Release\appLiftingTwinUI.exe --ui SystemMonitor

# 移动小车与云台控制
QtUI\build5\Release\appLiftingTwinUI.exe --ui TrolleyControl

# 短时轨迹预测视图
QtUI\build5\Release\appLiftingTwinUI.exe --ui TrajectoryPrediction
```

### 在其他电脑上构建

拉取代码后，确保上述前置要求满足，然后执行 `setup_and_build.bat` 即可。所有 Qt5Compat 兼容补丁已在脚本中自动处理，无需手动修改。

### 常见问题

| 问题 | 解决 |
|---|---|
| 窗口不显示 | 最小化终端窗口，或 Alt+Tab 切换到应用 |
| `模块"Qt5Compat.GraphicalEffects"没有安装` | 运行 `bash QtUI/third_party/setup_fluentui.sh` 手动重新打补丁，然后 `cmake --build build5 --config Release` |
| `找不到 Qt6Quick.dll` | 执行 `windeployqt --qmldir QtUI/qml QtUI/build5/Release/appLiftingTwinUI.exe` |
| 修改了 FluentUI 源码后 | 删除模块缓存 `cmake --build build5 --config Release --target clean`，然后完整重建 |
